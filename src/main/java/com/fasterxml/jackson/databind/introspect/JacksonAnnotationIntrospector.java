...
/**
 * {@link AnnotationIntrospector} implementation that handles standard Jackson
 * annotations.
 */
public class JacksonAnnotationIntrospector extends AnnotationIntrospector implements java.io.Serializable {
    ...
    /**
     * Annotations with meta-annotation {@link JacksonAnnotationsInside} are
     * considered bundles.
     */
    @Override
	...
	// 11-Jun-2015, tatu: As per [databind#677], need to allow explicit
	// naming.
	// Unfortunately can not quite use standard AnnotatedClass here (due to
	// various
	// reasons, including odd representation JVM uses); has to do for now
	try {
	    // We know that values are actually static fields with matching name
	    // so:
	...
	// 13-May-2015, tatu: As per [databind#95], allow read-only/write-only
	// props
	if (forSerialization) {
    ...
    /*
     * /********************************************************** /* General
     * member (field, method/constructor) annotations
     * /**********************************************************
     */
	...
	// Since annotations do not allow nulls, need to assume empty means
	// "none"
	...
	// that unwrapping should not be done (relevant when using chained
	// introspectors)
    ...
    public TypeResolverBuilder<?> findPropertyTypeResolver(MapperConfig<?> config, AnnotatedMember am,
	    JavaType baseType) {
	/*
	 * As per definition of @JsonTypeInfo, should only apply to contents of
	 * container (collection, map) types, not container types themselves:
	 */
	...
	/*
	 * 18-Oct-2010, tatu: [JACKSON-351] @JsonRawValue handled just here, for
	 * now; if we need to get raw indicator from other sources need to add
	...
	JsonSerialize ann = _findAnnotation(a, JsonSerialize.class);
	if (ann != null && ann.using() != JsonSerializer.None.class) {
	    return ann.nullsUsing();
	}
	return null;
    }
	...
	/*
	 * 23-Jun-2015, tatu: as per [databind#840], let's only consider `true`
    ...
    protected BeanPropertyWriter _constructVirtualProperty(JsonAppend.Attr attr, MapperConfig<?> config,
	...
	// now, then, we need a placeholder for member (no real Field/Method):
	AnnotatedMember member = new VirtualAnnotatedMember(ac, ac.getRawType(), propName.getSimpleName(),
		type.getRawClass());
	// and with that and property definition
	SimpleBeanPropertyDefinition propDef = SimpleBeanPropertyDefinition.construct(config, member, propName,
	...
	if (js != null) {
		 ...
		 * but it's actually bit tricky to do it more efficiently
		 * (meta-annotations add more lookups; AnnotationMap costs etc)
		 */
	    } else if (_hasAnnotation(a, JsonDeserialize.class) || _hasAnnotation(a, JsonView.class)
     ...
     * {@link TypeResolverBuilder} if given annotated element indicates one is
     * needed.
     */
    @SuppressWarnings("deprecation")
	...
	// 08-Dec-2014, tatu: To deprecated `JsonTypeInfo.None` we need to use
	// other placeholder(s);
	// and since `java.util.Void` has other purpose (to indicate "deser as
	// null"), we'll instead
	// use `JsonTypeInfo.class` itself. But any annotation type will
	// actually do, as they have no
    ...
    /**
     * Helper method for dealing with "no type info" marker; can't be null (as
     * it'd be replaced by default typing)
     */
